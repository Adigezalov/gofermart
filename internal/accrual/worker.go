...
func (w *Worker) processOrders() {
	...
	// Логируем только если есть заказы для обработки (но не каждый раз)
	// log.Printf("Проверяем статус %d заказов в системе начислений...", len(orders))
...
func (w *Worker) processOrder(ord *order.Order) error {
	...
	if err != nil {
		...
		case *OrderNotFoundError:
			// Заказ не зарегистрирован в системе начислений (204) - это нормально
			// Согласно спецификации: "Заказ может быть взят в расчёт в любой момент после его совершения"
			// Просто ждем, пока система начислений не подхватит заказ
			// Не логируем это как ошибку, чтобы не создавать шум в логах
			return nil
